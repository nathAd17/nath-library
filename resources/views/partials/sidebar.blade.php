...
<div class="fixed inset-y-0 left-0 z-50 w-64 p-1 shadow-lg shadow-lighthover my-2 mx-2 rounded-2xl bg-gradient-to-b from-blue-900 via-blue-600 to-blue-900 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0"
            ...
            <nav class="mt-6 px-4 space-y-2">
                <template x-for="item in navigation" :key="item.name">
                    <div>
                        <a :href="item.route"
                           class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 hover:bg-blue-800 hover:shadow-md transform hover:scale-105"
                           :class="item.active ? 'bg-blue-700 text-light shadow-lg' : 'text-blue-200 hover:text-light'">
                            ...
                            <span x-show="item.badge"
                                  class="ml-auto bg-red-500 text-light text-xs rounded-full px-2 py-1"
                                  x-text="item.badge"></span>
                        </a>

                        <!-- Submenu -->
                        <div x-show="item.submenu && item.active"
                            ...
                            <template x-for="subitem in item.submenu" :key="subitem.name">
                                <a :href="subitem.route"
                                   class="block px-4 py-2 text-sm text-blue-200 hover:text-light hover:bg-blue-800 rounded-lg transition-colors duration-200"
                                   :class="subitem.active ? 'bg-blue-700 text-light' : ''"
            ...
            <div class="absolute bottom-0 left-0 right-0 p-3">
                <div class="flex items-center space-x-3 hover:-translate-y-1 ease-in-out transition-transform h-16 p-4 bg-light/10 backdrop-blur-lg rounded-xl shadow-blue-950/50 shadow-lg">
